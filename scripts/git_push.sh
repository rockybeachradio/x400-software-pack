...
# Date: 20250925
# Purpose: Uploads the repo folder (only changes) to the GitHub repository
################################################################################################
...
if git diff --cached --quiet; then
  ...
  echo "StagingArea --> Local Repository: git commit ..."
...
if git rev-parse --abbrev-ref --symbolic-full-name "@{u}" >/dev/null 2>&1; then
  echo "Rebase on remote: git pull --rebase --autostash ..."
...
################################################################################################
if [[ -n "$TAG" ]]; then
  echo "Print URL to draft a release on GitHub ..."
