...
PA2: 调试口输出，焊点或者排针  -  Translation: Debug port output, solder joint or pin header
...
//OUT引脚输入输出 方向设置  PA3  -  Translation: OUT pin input and output direction setting PA3
...
/*	GPIO_InitTypeDef  GPIO_InitStructure;					
	...
	// PA2 ----- CLK  设置为输出  -  Translation: CLK is set to output
	// PA3 ----- OUT  设置为输出  -  Translation: OUT Set as output
	GPIO_InitStructure.GPIO_Pin = HX711_SCK_Pin | HX711_Dout_Pin;		
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		
	GPIO_Init(GPIOA, &GPIO_InitStructure);
		
	GPIO_SetBits(GPIOA, HX711_SCK_Pin);	// CLK 拉高  -  Translation: CLK is pulled high
	GPIO_SetBits(GPIOA, HX711_Dout_Pin);	// OUT 拉高  -  Translation: OUT pull high
...
//配置CS1237芯片  -  Translation: Configure CS1237 chip
void CS1237_Config(void)
{
	unsigned char i;
	unsigned char dat;
	unsigned int count_i=0;//溢出计时器  - Translation: Overflow Timer
	
	dat = 0X2C;   //芯片地配置 内部REF 输出40HZ PGA=128 通道A 0X1C   -  Translation: Chip ground configuration Internal REF output 40HZ PGA=128 Channel A 0X1C
	OUT_OUT();
	GPIO_SetBits(GPIOA, HX711_Dout_Pin); //OUT 引脚拉高  -  Translation: OUT pin pulled high
	OUT_IN();
	GPIO_ResetBits(GPIOA, HX711_SCK_Pin);// 时钟拉低  -  Tranlsation: Clock pulled low
	while(HAL_GPIO_ReadPin(GPIOA, HX711_Dout_Pin)==1) //等待CS237准备好  -  Translation: Waiting for CS237 to be ready
	{
		delay_ms(1);
		count_i++;
		if(count_i > 300)
		{
			OUT_OUT();
			GPIO_SetBits(GPIOA, HX711_Dout_Pin); // OUT 引脚拉高  -  Translation: OUT pin pulled high
			GPIO_SetBits(GPIOA, HX711_SCK_Pin); // CLK 引脚拉高  -  Translation: CLK pin pulled high
			return;//超时，则直接退出程序  -  Translation: If timeout occurs, exit the program directly
		}
	}
	for(i=0;i<29;i++)// 1 - 29
	{
	...
	//37     写入了0x65  -  Translation: 0x65 is written
	GPIO_SetBits(GPIOA, HX711_SCK_Pin);	// CLK=1;
	Delay1us();
	GPIO_ResetBits(GPIOA, HX711_SCK_Pin);	// CLK=0;
	Delay1us();
	
	for(i=0;i<8;i++)// 38 - 45个脉冲了，写8位数据  -  Translation: 38 - 45 pulses, write 8 bits of data
	{
		GPIO_SetBits(GPIOA, HX711_SCK_Pin);	// CLK=1;
		Delay1us();
		if(dat&0x80){
...
// 读取芯片的配置数据  -  Translation: Read chip configuration data
