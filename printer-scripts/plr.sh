...
# 1) For Debug:
    ...
    echo ${2}           # full patch to the original G-Code file to resume. /eg /home/mks/printer_data/gcodes/part.gcode)

# 2) Prepare files in tmp folder
    #rm /tmp/plr.gcode
    #z_s=$(echo ${1})
    #z_p=${1} #$("${1}" | sed 's/\./\\./g')
...
# 5) setup resume commands. Insert gcode commands into the plr.gcode file.
    ...
    echo 'G28 X' >> /tmp/plr.gcode                          # home XY (not Z!)
